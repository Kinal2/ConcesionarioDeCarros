    ...
    if (methodType == MethodType.POST) {
        ...
        try {
            if (concesionarioAlert.mostrarAlertaConfirmacion(106).get() == ButtonType.OK) {
                Venta ventaOriginal = buscarVentaPorId(venta.getId());
                
                if (ventaOriginal != null && ventaOriginal.getCarro().getId().equals(venta.getCarro().getId())) {
                    concesionarioAlert.mostrarAlertaInfo(401);
                    return ventaRepository.save(venta);
                } else if (!verificarEstadoCarro(venta)) {
                    concesionarioAlert.mostrarAlertaInfo(401);
                    return ventaRepository.save(venta);
                } else {
                    concesionarioAlert.mostrarAlertaInfo(501);
